...
function StatCard({ label, value, sub, icon, accent }) {
  return (
    <div className="bg-turf-surface border border-turf-border rounded-xl p-4">
      ...
      <p className={`font-display font-black text-2xl ${accent ? '' : 'text-white'}`} style={accent ? { color: accent } : {}}>
        {value}
      </p>
      <p className="text-turf-muted text-xs uppercase tracking-wider mt-0.5">{label}</p>
      ...
      <div className="flex-shrink-0 px-4 pt-6 pb-4 border-b border-turf-border">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ backgroundColor: `${user.color}20`, borderColor: user.color, color: user.color }}
          >
            {user.username[0].toUpperCase()}
          </div>
          <div>
            <h1 className="font-display font-black text-xl text-white">{user.username}</h1>
            <p className="text-turf-muted text-xs">Runner since {new Date(user.createdAt || Date.now()).toLocaleDateString()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <StatCard
            icon="📅"
            label="Last Run"
            value={user.lastRunDate ? new Date(user.lastRunDate).toLocaleDateString() : 'Never'}
          />
        </div>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="bg-turf-surface border border-turf-border rounded-xl p-6 text-center">
              ...
              <p className="text-turf-muted text-sm mt-2">No runs yet. Hit the streets!</p>
            </div>
          ) : (
              ...
              {runs.map(run => (
                <div key={run._id} className="bg-turf-surface border border-turf-border rounded-xl p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-turf-muted text-xs mt-0.5">
                        {new Date(run.startTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-turf-border/50">
                    <div>
                      <p className="font-mono font-semibold text-sm text-white">{formatDistance(run.distance || 0)}</p>
                      <p className="text-turf-muted text-[10px] uppercase">Distance</p>
                    </div>
                    <div>
                      <p className="font-mono font-semibold text-sm text-white">{formatDuration(run.duration || 0)}</p>
                    ...
                    <div>
                      <p className="font-mono font-semibold text-sm text-white">
                        {run.distance && run.duration
                          ? `${Math.floor((run.duration / run.distance) * 1000 / 60)}:${String(Math.floor(((run.duration / run.distance) * 1000) % 60)).padStart(2, '0')}`
                          : '--:--'}
                      </p>
